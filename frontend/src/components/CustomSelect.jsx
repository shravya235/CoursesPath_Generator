...
const CustomSelect = ({ options, value, onChange, placeholder }) => {
  ...
  return (
    <div className="relative" ref={selectRef}>
      ...
      {isOpen && (
        <div className="absolute z-20 w-full mt-2 bg-white border border-gray-300 rounded-lg shadow-xl overflow-hidden">
          {options.map((option) => (
            <div
              key={option.value}
              onClick={() => handleSelect(option.value)}
              className="px-4 py-3 text-gray-900 hover:bg-cyan-50 cursor-pointer transition-colors"
            >
              {option.label}
            </div>
          ))}
